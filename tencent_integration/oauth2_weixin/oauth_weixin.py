...
def get_info_via_oauth(
	...
	if id_token:
		...
		if provider == "weixinmini":
			info = json.loads(session.access_token_response.content)
		else:
...
def login_oauth_user(
	...
	if frappe.utils.cint(generate_login_token):
		frappe.log_error('generate login token')
		login_token = frappe.generate_hash(length=32)
		frappe.log_error('token',login_token)
...
def get_first_name(data: dict) -> str:
	return data.get("first_name") or data.get("given_name") or data.get("name") or data.get("nickname") or data.get("openid")
